...
- **For Step 6**: In RobotContainer.java, create an instance of your subsystem in the class variables. In `configureButtonBindings()`, use `driverController.a().onTrue(new StartMotorCommand(motorSubsystem));` and `driverController.b().onTrue(new StopMotorCommand(motorSubsystem));`
